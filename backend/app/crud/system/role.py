...
async def get_role_by_id(db: AsyncSession, role_id: int) -> Optional[Role]:
    ...
    result = await db.execute(
        select(Role).options(selectinload(Role.permissions).selectinload(RoleMenu.menu)).where(Role.id == role_id)
    )
...
async def get_roles(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Role]:
    """获取角色列表"""
    result = await db.execute(
        select(Role).options(selectinload(Role.permissions).selectinload(RoleMenu.menu))
